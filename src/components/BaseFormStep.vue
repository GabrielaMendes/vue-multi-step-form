...
<template>
  <div class="h-full flex flex-col p-4 max-[375px]:p-3  md:pt-10 md:pb-5 md:px-[15%]">
    <section>
      <h1 class="font-bold mb-2 max-[375px]:mb-0 text-[26px] md:text-[34px] text-marine-blue">
        <slot name="title"></slot>
      </h1>
      <p><slot name="description"></slot></p>
    </section>

    <section class="mt-8 max-[375px]:mt-6 md:mt-10">
      <slot name="main"></slot>
    </section>

    <teleport to="body" :disabled="isDesktop">
      <section class="absolute bottom-0 left-0 w-full flex justify-between mt-auto bg-white p-5 max-[375px]:p-3 md:p-0 md:static">
        <button
          v-show="!isFirst"
          type="button"
          @click="goBack"
